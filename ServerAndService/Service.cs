...
namespace ServerAndService
{
    ...
    internal class Service
    {
        ...
        // Đặt lại mật khẩu
        public async Task<string> ResetPassword(string email, string newPasswordHash)
        {
            try
            {
                var result = await client.Rpc("reset_password", new
                {
                    _email = email,
                    _new_password = newPasswordHash
                });
            ...
            {
                return $"ERROR_RESET_PASSWORD: {ex.Message}";
            }
        }
    }
}
