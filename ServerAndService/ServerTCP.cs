            ...
            try
            {
                ...
                switch (command)
                {
                    ...
                    case "POST_REVIEW":
                        ...
                        break;
                    // case dùng để đặt lại mật khẩu
                    case "RESET_PASSWORD":
                        if (parts.Length < 3)
                        {
                            response = "ERROR: RESET_PASSWORD requires email and new password hash";
                        }
                        else
                        {
                            ...
                            response = await service.ResetPassword(email, newPasswordHash);
                        }
                        break;
                    default:
                        // sửa lỗi hiển thị lệnh không xác định
